...
<DocsLayout title="API Reference" description="HTTP API reference for the RunOwl webhook server.">
	<div class="prose">
		<h1>API Reference</h1>
		<p class="page-lead">
			RunOwl exposes a small HTTP API when running as a server. The webhook endpoint receives GitHub
			events; the utility endpoints are useful for monitoring and debugging.
		</p>
		...
		<div class="code-block">
			...
			<pre><code
					>http://localhost:8000   <span style="color:var(--muted)"># default local</span>
https://your-server.com <span style="color:var(--muted)"># production</span></code
				></pre>
		</div>
  ...
  "version": "0.1.0"
		...
		<p>
			GitHub webhook receiver. Accepts <code>pull_request</code> events and queues reviews as background
			jobs.
		</p>
		...
		<table>
			...
			<tbody>
				...
				<tr
					><td><code>200 OK</code></td><td>Event received but ignored (e.g. non-review action)</td
					></tr
				>
		...
		<div class="callout info">
			...
			<p>
				The server always returns immediately. The review runs asynchronously in the background.
				Results are posted as PR comments and Check Run updates.
			</p>
		</div>
      ...
      "blocks_merge": true       // true for P0 and P1
		...
		<table>
			...
			<tbody>
				...
				<tr
					><td><code>investigation</code></td><td
						>Requires human investigation; RunOwl flagged a concern but cannot confirm</td
					></tr
				>
				<tr
					><td><code>informational</code></td><td>P3 advisory — style, naming, minor improvement</td
					></tr
				>
			</tbody>
		</table>
		...
		<h2>Rate Limits</h2>
		<p>
			RunOwl imposes no rate limits of its own on the HTTP API. Rate limiting comes from upstream
			services:
		</p>
		<ul>
		...
		<div class="code-block">
			<div class="code-block-header"></div>
			<pre><code
					>http://localhost:8000/docs     <span style="color:var(--muted)"># Swagger UI</span>
http://localhost:8000/redoc    <span style="color:var(--muted)"># ReDoc</span>
http://localhost:8000/openapi.json  <span style="color:var(--muted)"># Raw schema</span></code
				></pre>
		</div>
	</div>
</DocsLayout>
