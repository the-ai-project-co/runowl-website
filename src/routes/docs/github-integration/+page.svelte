...
>
	<div class="prose">
		...
		<p class="page-lead">
			RunOwl integrates with GitHub in two ways: automatic reviews via webhooks, and status feedback
			via Check Runs. Set it up once and every PR gets reviewed automatically.
		</p>

		<h2>Webhook Setup</h2>
		<p>
			The webhook server listens for <code>pull_request</code> events and triggers a review whenever a
		...
		<div class="code-block">
			...
			<pre><code
					><span style="color:var(--muted)"># With uv (recommended)</span>
...
uvicorn main:app --host 0.0.0.0 --port 8000</code
		...
		<ol class="step-list">
			...
			<li>
				<div>
					Set <strong>Payload URL</strong> to your server:
					<code>https://your-server.com/webhook/github</code>
				</div>
			</li>
			<li>
			...
			<li>
				<div>
					Generate a secret: <code>openssl rand -hex 32</code> — copy it as <strong>Secret</strong>,
					and set it as <code>GITHUB_WEBHOOK_SECRET</code> in your environment
				</div>
			</li>
			<li>
				<div>
					Under <strong>Which events?</strong>, select
		...
		<div class="callout warning">
			...
			<p>
				<strong>Always set a webhook secret.</strong> Without it, anyone can send fake events to
				your server. RunOwl verifies the <code>X-Hub-Signature-256</code> header when
				<code>GITHUB_WEBHOOK_SECRET</code> is set.
			</p>
		</div>
		...
		<div class="code-block">
			...
			<pre><code
					>GEMINI_API_KEY=AIza...
GITHUB_TOKEN=ghp_...
GITHUB_WEBHOOK_SECRET=your-secret-here</code
				></pre>
		</div>
		...
		<p>
			All other actions (assigned, labeled, closed, etc.) are silently ignored. The server responds <code
				>202 Accepted</code
			> immediately and runs the review as a background job.
		</p>

		<h2>GitHub Check Runs <span class="pill pill-paid">Team</span></h2>
		<p>
			With a Team subscription, RunOwl creates a GitHub Check Run on every PR — giving you a
			pass/fail status directly in the PR interface.
		</p>
		...
		<ol class="step-list">
			<li>
				<div>
					When a PR opens, RunOwl immediately creates a check run with status <strong
						>in_progress</strong
					>. The PR shows "RunOwl Code Review — checking..." in the Checks section.
				</div>
			</li>
			<li>
			...
			<li>
				<div>
					If branch protection rules require the RunOwl check to pass, P0/P1 findings <strong
						>block the merge button</strong
					>.
				</div>
			</li>
		</ol>
...
---
*3 findings · 1 blocking · [View full trace](https://runowl.ai)*</code
				></pre>
		</div>
		...
		<div class="callout tip">
			...
			<p>
				For GitHub Actions workflows, use the automatically-provided <code
					>secrets.GITHUB_TOKEN</code
				>. It has all required permissions by default. No need to create a PAT.
			</p>
		</div>

		<h2>GitHub App (Advanced)</h2>
		<p>
			For organisation-wide deployment, you can install RunOwl as a GitHub App instead of using a
			PAT. This provides better security, clearer audit trails, and per-repo installation control.
		</p>

		<div class="code-block">
			<div class="code-block-header">.env</div>
			<pre><code
					><span style="color:var(--muted)"># GitHub App credentials</span>
...
GITHUB_WEBHOOK_SECRET=your-secret-here</code
		...
		<ul>
			<li>
				<strong>LRU caching</strong> — file content is cached (200 entries) to avoid re-fetching the same
				files across tool calls
			</li>
			<li><strong>Concurrency limit</strong> — max 5 concurrent GitHub API requests</li>
			<li>
				<strong>Exponential backoff</strong> — automatic retries on 429 and 5xx responses (via tenacity)
			</li>
