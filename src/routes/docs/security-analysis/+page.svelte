...
>
	<div class="prose">
		...
		<p class="page-lead">
			RunOwl performs security analysis in two tiers: fast pattern-based surface checks available to
			everyone, and deep OWASP-aligned analysis for paid subscribers.
		</p>

		<h2>Surface Checks <span class="pill pill-free">Free</span></h2>
		<p>
			Pattern-matching rules applied to every added line in the diff. Fast, deterministic, and zero
			false-negative on known patterns.
		</p>
		...
		<div class="callout info">
			...
			<p>
				Surface checks run against <strong>added lines only</strong>. Unchanged code is not scanned
				— only what your PR actually introduced.
			</p>
		</div>

		<h2>Deep Security Analysis <span class="pill pill-paid">Team</span></h2>
		<p>
			Enable with <code>--expert</code> flag. Performs OWASP Top 10-aligned analysis through the Reasoning
			Loop — the model investigates code paths, not just patterns.
		</p>
		...
		<table>
			...
			<tbody>
				...
				<tr>
					...
					<td
						>Weak JWT (none algorithm, HS256 with exposed secret), session fixation, improper logout</td
					>
				</tr>
				<tr>
		...
		<table>
			...
			<tbody>
				<tr>
					...
					<td
						>TOCTOU vulnerabilities, non-atomic check-then-act, concurrent resource access without
						locks</td
					>
				</tr>
				<tr>
		...
		<p>
			Security findings follow a strict severity floor — the reclassifier ensures these are never
			downgraded:
		</p>
		...
		<table>
			...
			<tbody>
				<tr
					><td>SQL injection, RCE, auth bypass, path traversal, XXE, SSRF</td><td
						><span class="sev-badge sev-p0">P0</span></td
					></tr
				>
				<tr
					><td>XSS, CSRF, open redirect, weak JWT, race condition (TOCTOU)</td><td
						><span class="sev-badge sev-p1">P1</span></td
					></tr
				>
				<tr
					><td>CORS misconfiguration, missing security headers, weak cipher</td><td
						><span class="sev-badge sev-p2">P2</span></td
					></tr
				>
				<tr
					><td>Unpinned deps, informational security notes</td><td
						><span class="sev-badge sev-p3">P3</span></td
					></tr
				>
			</tbody>
		</table>
		...
		<div class="callout danger">
			...
			<p>
				<strong>P0 and P1 findings block the merge</strong> when GitHub Check Runs are configured. Your
				team cannot merge a PR with an exposed secret or an auth bypass — period.
			</p>
		</div>

		<h2>Architecture Analysis <span class="pill pill-paid">Team</span></h2>
		<p>
			Alongside deep security, <code>--expert</code> also enables SOLID principle analysis and code smell
			detection.
		</p>
		...
		<table>
			...
			<tbody>
				<tr
					><td>Single Responsibility</td><td
						>Classes doing too many things; methods with multiple distinct concerns</td
					></tr
				>
				<tr
					><td>Open/Closed</td><td
						>Code that requires modification to extend; missing abstraction points</td
					></tr
				>
				<tr
					><td>Liskov Substitution</td><td>Subclass overrides that break parent class contracts</td
					></tr
				>
				<tr
					><td>Interface Segregation</td><td
						>Fat interfaces forcing implementations to stub unused methods</td
					></tr
				>
				<tr
					><td>Dependency Inversion</td><td
						>High-level modules depending on concrete implementations</td
					></tr
				>
			</tbody>
		</table>
		...
		<ul>
			<li>
				<strong>God objects</strong> — classes with too many responsibilities and dependencies
			</li>
			...
			<li>
				<strong>Feature envy</strong> — methods that use another class's data more than their own
			</li>
			<li>
				<strong>Primitive obsession</strong> — over-use of primitives where value objects belong
			</li>
		</ul>
  ...
  --expert</code
		...
		<p>
			Without a valid API key, <code>--expert</code> will note that deep analysis requires a Team subscription
			and fall back to surface checks only.
		</p>
	</div>
</DocsLayout>
